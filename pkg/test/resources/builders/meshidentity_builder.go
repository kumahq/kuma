...
func (mi *MeshIdentityBuilder) WithBundledCA(cert []byte, key []byte) *MeshIdentityBuilder {
	mi.WithBundled()
	mi.res.Spec.Provider.Bundled.CA = &meshidentity_api.CA{
		Certificate: &datasource.SecureDataSource{
			Type:           datasource.SecureDataSourceInline,
			InsecureInline: &datasource.Inline{Value: string(cert)},
		},
		PrivateKey: &datasource.SecureDataSource{
			Type:           datasource.SecureDataSourceInline,
			InsecureInline: &datasource.Inline{Value: string(key)},
		},
	}
	return mi
}
