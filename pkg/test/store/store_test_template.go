...
func ExecuteStoreTests(
	createStore func(name string) store.ResourceStore,
	storeName string,
) {
	...
	BeforeEach(func() {
		s = store.NewStrictResourceStore(store.NewPaginationStore(createStore(storeName)))
	})
	...
	Context("Store: "+storeName, func() {
		...
		Describe("Update()", func() {
			...
			It("should update an existing resource", func() {
				...
				if reflect.TypeOf(createStore(storeName)) != reflect.TypeOf(&resources_k8s.KubernetesStore{}) {
