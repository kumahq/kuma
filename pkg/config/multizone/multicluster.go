...
func (r *ZoneConfig) Validate() error {
	...
	if violations := apimachineryvalidation.NameIsDNS1035Label(r.Name, false); len(violations) > 0 {
		return errors.Errorf("Zone name %s is invalid: %s", r.Name, strings.Join(violations, "; "))
	}
	if r.GlobalAddress != "" {
