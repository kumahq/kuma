...
var _ = Describe("Default config", func() {
	...
	It("should not full-resync KDS snapshots every flush interval", func() {
		cfg := kuma_cp.DefaultConfig()

		Expect(cfg.Experimental.KDSEventBasedWatchdog.FlushInterval.Duration).To(Equal(time.Second))
		Expect(cfg.Experimental.KDSEventBasedWatchdog.FullResyncInterval.Duration).To(Equal(time.Minute))
		Expect(cfg.Experimental.KDSEventBasedWatchdog.FullResyncInterval.Duration).To(
			BeNumerically(">", cfg.Experimental.KDSEventBasedWatchdog.FlushInterval.Duration),
		)
	})
