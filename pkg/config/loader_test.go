...
var _ = Describe("Config loader", func() {
	...
	DescribeTable("should load config",
		func(given testCase) {
			...
			Expect(cfg.Metrics.OpenTelemetry.ExporterInitTimeout.Duration).To(Equal(2 * time.Second))
			Expect(cfg.Metrics.OpenTelemetry.ShutdownTimeout.Duration).To(Equal(10 * time.Second))
			...
			Expect(cfg.CoreResources.Status.MeshOpenTelemetryBackendInterval.Duration).To(Equal(10 * time.Second))
		},
...
coreResources:
  ...
  status:
    ...
    workloadInterval: 9s
    meshOpenTelemetryBackendInterval: 10s
policies:
...
`,
		...
		Entry("from env variables", testCase{
			envVars: map[string]string{
				...
				"KUMA_METRICS_OPEN_TELEMETRY_EXPORTER_INIT_TIMEOUT":                                       "2s",
				"KUMA_METRICS_OPEN_TELEMETRY_SHUTDOWN_TIMEOUT":                                            "10s",
				...
				"KUMA_CORE_RESOURCES_STATUS_MESH_OPEN_TELEMETRY_BACKEND_INTERVAL":                          "10s",
