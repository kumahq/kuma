...
var _ = Describe("Config loader", func() {
	...
	DescribeTable("should load config",
		func(given testCase) {
			...
			Expect(cfg.Runtime.Kubernetes.WatchNamespaces).To(Equal([]string{"namespace-1", "namespace-2"}))
...
runtime:
  ...
  kubernetes:
    ...
    watchNamespaces: ["namespace-1", "namespace-2"]
reports:
  enabled: false
general:
  tlsKeyFile: /tmp/key
...
`,
		...
		Entry("from env variables", testCase{
			envVars: map[string]string{
				...
				"KUMA_RUNTIME_KUBERNETES_WATCH_NAMESPACES":                                                 "namespace-1,namespace-2",
