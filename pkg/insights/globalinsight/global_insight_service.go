...
// to. Falls back to the Dataplane itself when it carries no workload label, which
// keeps every gateway counted. Each tier is prefixed so that gateways grouped by
// different tiers never share a key.
