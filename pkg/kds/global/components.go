...
import (
	...
	"github.com/kumahq/kuma/v2/pkg/kds/client"
	"github.com/kumahq/kuma/v2/pkg/kds/mux"
	kds_server "github.com/kumahq/kuma/v2/pkg/kds/server"
	"github.com/kumahq/kuma/v2/pkg/kds/service"
	sync_store "github.com/kumahq/kuma/v2/pkg/kds/store"
...
func Setup(rt runtime.Runtime) error {
	...
	onZoneToGlobalSyncConnect := mux.OnZoneToGlobalSyncConnectFunc(func(stream mesh_proto.KDSSyncService_ZoneToGlobalSyncServer, errChan chan error) {
		...
		log := kdsDeltaGlobalLog.WithValues("peer-id", zoneID)
		log = kuma_log.AddFieldsFromCtx(log, stream.Context(), rt.Extensions())
		kdsStream := kds_client_v2.NewDeltaKDSStream(kds_client_v2.NewServerStreamAdapter(stream), zoneID, rt, "", len(reg.ObjectTypes(model.HasKDSFlag(model.ZoneToGlobalFlag))))
