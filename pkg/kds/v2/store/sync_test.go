...
// another writer changed the resource in between, which on the Zone is what the
// VIP allocator and the hostname generators do to the resources we sync.
...
var _ = Describe("SyncResourceStoreDelta write conflicts", func() {
	...
	// changedMesh is meshBuilder(1) with a different spec, so syncing it against a
	// stored meshBuilder(1) produces an update rather than a create.
	...
	It("should retry the update and apply the change after losing a conflict", func() {
		...
		// the change landed rather than being dropped, and it took a second attempt
		actual := mesh.NewMeshResource()
		Expect(resourceStore.Get(context.Background(), actual, store.GetBy(key))).To(Succeed())
	...
	It("should give up and return the error when conflicts do not stop", func() {
		// the caller tears the stream down on this error, which forces a full
		// resync. Silently skipping the resource would let the zone drift.
