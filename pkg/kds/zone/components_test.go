...
var _ = Describe("Zone Sync", func() {
	...
	newPolicySink := func(zoneName string, resourceSyncer sync_store.ResourceSyncer, cs *grpc.MockClientStream, rt core_runtime.Runtime) component.Component {
		return kds_client.NewKDSSink(core.Log, registry.Global().ObjectTypes(model.KdsFlagFilter(model.ConsumedByZone)), kds_client.NewKDSStream(cs, zoneName), zone.Callbacks(rt, resourceSyncer, false, zoneName, nil))
	}
	...
	BeforeEach(func() {
		...
		serverStream := setup.StartServer(globalStore, wg, "global", registry.Global().ObjectTypes(model.KdsFlagFilter(model.ConsumedByZone)), kdsCtx.GlobalProvidedFilter)
	...
	It("should have up to date list of consumed types", func() {
		...
		actualConsumedTypes := registry.Global().ObjectTypes(model.ScopeFilter(model.ScopeMesh), model.TypeFilterFn(func(descriptor model.ResourceTypeDescriptor) bool {
		...
		Expect(actualConsumedTypes).To(ConsistOf(registry.Global().ObjectTypes(model.KdsFlagFilter(model.ConsumedByZone))))
	})
