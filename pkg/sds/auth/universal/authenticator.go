...
func validateType(dataplane *mesh.DataplaneResource, dpType builtin_issuer.DpType) error {
	...
	if dataplane.Spec.IsIngress() && dpType != builtin_issuer.DpTypeIngress {
		return errors.Errorf("dataplane is of type Ingress but token allows for the %q type", dpType)
	}
	if !dataplane.Spec.IsIngress() && dpType == builtin_issuer.DpTypeIngress {
		return errors.Errorf("dataplane is of type Dataplane but token allows for the %q type", dpType)
	}
	return nil
}
