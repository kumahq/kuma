...
var _ = Describe("MeshSnapshot Cache", func() {
	...
	BeforeEach(func() {
		...
		meshCache = mesh.NewCache(countingRm, expiration,
	...
	BeforeEach(func() {
		for i := 0; i < 3; i++ {
			...
			err := s.Create(context.Background(), &mesh_core.MeshResource{}, core_store.CreateByKey(mesh, mesh))
	...
	It("should cache a hash of mesh", func() {
		...
		Expect(countingRm.listQueries).To(Equal(4))
	})
