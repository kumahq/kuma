...
var _ = Describe("MeshSnapshot Cache", func() {
	...
	BeforeEach(func() {
		...
		meshContextBuilder := xds_context.NewMeshContextBuilder(countingManager, []core_model.ResourceType{core_mesh.DataplaneType, core_mesh.TrafficRouteType, core_mesh.ZoneIngressType}, lookupIPFunc, "zone-1", vips.NewPersistence(core_manager.NewResourceManager(s), manager.NewConfigManager(s)), "mesh")
	...
	It("should cache a hash of mesh", func() {
		...
		Expect(countingManager.listQueries).To(MatchAllKeys(Keys{
			core_mesh.DataplaneType:    Equal(2), // one for all, one for builtin Gateway
		...
		Expect(countingManager.listQueries).To(MatchAllKeys(Keys{
			core_mesh.DataplaneType:    Equal(4), // one for all, one for builtin Gateway
	...
	It("should retry previously failed Get() requests", func() {
		...
		Expect(err).ToNot(HaveOccurred())
		Expect(meshCtx.Hash).To(Equal(expectedHash))
		Expect(countingManager.getQueries).To(Equal(3))           // same as above
		Expect(countingManager.listQueries).To(MatchAllKeys(Keys{ // same as above
			core_mesh.DataplaneType:    Equal(2),
			core_mesh.TrafficRouteType: Equal(1),
			core_mesh.ZoneIngressType:  Equal(1),
