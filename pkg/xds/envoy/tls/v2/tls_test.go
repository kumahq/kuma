...
import (
	...
	v2 "github.com/kumahq/kuma/pkg/xds/envoy/tls/v2"
...
var _ = Describe("CreateDownstreamTlsContext()", func() {
	...
	Context("when mTLS is disabled on a given Mesh", func() {
		...
		It("should return `nil`", func() {
			...
			// when
			snippet, err := v2.CreateDownstreamTlsContext(ctx, metadata)
			// then
	...
	Context("when mTLS is enabled on a given Mesh", func() {
		...
		DescribeTable("should generate proper Envoy config",
			func(given testCase) {
				...
				// when
				snippet, err := v2.CreateDownstreamTlsContext(ctx, given.metadata)
				// then
				Expect(err).ToNot(HaveOccurred())
				// when
...
var _ = Describe("CreateUpstreamTlsContext()", func() {
	...
	Context("when mTLS is disabled on a given Mesh", func() {
		...
		It("should return `nil`", func() {
			...
			// when
			snippet, err := v2.CreateUpstreamTlsContext(ctx, metadata, "backend", "backend")
			// then
			Expect(err).ToNot(HaveOccurred())
			// and
			Expect(snippet).To(BeNil())
	...
	Context("when mTLS is enabled on a given Mesh", func() {
		...
		DescribeTable("should generate proper Envoy config",
			func(given testCase) {
				...
				// when
				snippet, err := v2.CreateUpstreamTlsContext(ctx, given.metadata, given.upstreamService, "")
				// then
				Expect(err).ToNot(HaveOccurred())
				// when
				actual, err := util_proto.ToYAML(snippet)
