...
func CreateDownstreamTlsContext(ctx xds_context.Context, metadata *core_xds.DataplaneMetadata) (*envoy_tls_v3.DownstreamTlsContext, error) {
...
func CreateUpstreamTlsContext(ctx xds_context.Context, metadata *core_xds.DataplaneMetadata, upstreamService string, sni string) (*envoy_tls_v3.UpstreamTlsContext, error) {
	if !ctx.Mesh.Resource.MTLSEnabled() {
		return nil, nil
	}
	var validationSANMatcher *envoy_type_matcher_v3.StringMatcher
	if upstreamService == "*" {
		validationSANMatcher = MeshSpiffeIDPrefixMatcher(ctx.Mesh.Resource.Meta.GetName())
	} else {
...
func createCommonTlsContext(ctx xds_context.Context, metadata *core_xds.DataplaneMetadata, validationSANMatcher *envoy_type_matcher_v3.StringMatcher) (*envoy_tls_v3.CommonTlsContext, error) {
		...
		ValidationContextType: &envoy_tls_v3.CommonTlsContext_CombinedValidationContext{
			CombinedValidationContext: &envoy_tls_v3.CommonTlsContext_CombinedCertificateValidationContext{
				DefaultValidationContext: &envoy_tls_v3.CertificateValidationContext{
					MatchSubjectAltNames: []*envoy_type_matcher_v3.StringMatcher{validationSANMatcher},
				},
...
func sdsSecretConfig(context xds_context.Context, name string, metadata *core_xds.DataplaneMetadata) (*envoy_tls_v3.SdsSecretConfig, error) {
	withCallCredentials := func(grpc *envoy_core_v3.GrpcService_GoogleGrpc) (*envoy_core_v3.GrpcService_GoogleGrpc, error) {
		if metadata.GetDataplaneTokenPath() == "" {
			return grpc, nil
		}

		config := &envoy_grpc_credential_v3.FileBasedMetadataConfig{
		...
		grpc.CallCredentials = append(grpc.CallCredentials, &envoy_core_v3.GrpcService_GoogleGrpc_CallCredentials{
	...
	googleGrpc, err := withCallCredentials(&envoy_core_v3.GrpcService_GoogleGrpc{
...
func UpstreamTlsContextOutsideMesh(ca, cert, key []byte, hostname string) (*envoy_tls_v3.UpstreamTlsContext, error) {
	var tlsCertificates []*envoy_tls_v3.TlsCertificate
	if cert != nil && key != nil {
...
func dataSourceFromBytes(bytes []byte) *envoy_core_v3.DataSource {
...
func MeshSpiffeIDPrefixMatcher(mesh string) *envoy_type_matcher_v3.StringMatcher {
...
func ServiceSpiffeIDMatcher(mesh string, service string) *envoy_type_matcher_v3.StringMatcher {
	return &envoy_type_matcher_v3.StringMatcher{
		MatchPattern: &envoy_type_matcher_v3.StringMatcher_Exact{
