...
`,
		...
		Entry("basic tcp_proxy with weighted destination clusters", testCase{
			...
			statsName:       "db",
			clusters: []envoy_common.Cluster{
				envoy_common.NewCluster(envoy_common.WithName("db-0"), envoy_common.WithWeight(10)),
				envoy_common.NewCluster(envoy_common.WithName("db-1"), envoy_common.WithWeight(90)),
			},
			expected: `
            address:
              socketAddress:
