...
var _ = Describe("Authentication flow", func() {
	...
	It("should throw an error when signing key is not found", func() {
		...
		// then
		Expect(err).To(MatchError(`there is no Signing Key in the Control Plane for Mesh "demo". Make sure the Mesh exist. If you run multi-zone setup, make sure Zone is connected to the Global before generating tokens.`))
	})
})
