...
var _ = Describe("Secrets", Ordered, func() {
	...
	Context("dataplane proxy", func() {
		...
		Context("mTLS identity of legacy dataplanes carrying inbound service tags", func() {
			newLegacyDataplane := func(labels map[string]string) *core_mesh.DataplaneResource {
				dp := newDataplane()
				dp.Meta.(*model.ResourceMeta).Labels = labels
				dp.Spec.Networking.Inbound[0].Tags = map[string]string{
					"kuma.io/service": "web-legacy",
				}
				return dp
			}

			It("should derive identity from the inbound service tag when there is no workload label", func() {
				// given a legacy dataplane with inbound tags and no workload label
				dataplane := newLegacyDataplane(nil)

				// when
				identity, ca, err := secrets.GetForDataPlane(context.Background(), dataplane, newMesh("default"), nil)

				// then a cert is issued under the tag's identity, so the proxy
			...
			It("should prefer the inbound service tag over the workload label", func() {
				// given a dataplane carrying both
				dataplane := newLegacyDataplane(map[string]string{"kuma.io/workload": "web"})

				// when
				_, _, err := secrets.GetForDataPlane(context.Background(), dataplane, newMesh("default"), nil)

				// then the explicit tag stays the identity, so its SAN doesn't
			...
			It("GetAllInOne should derive identity from the inbound service tag", func() {
				// given a legacy dataplane with inbound tags and no workload label
				dataplane := newLegacyDataplane(nil)
