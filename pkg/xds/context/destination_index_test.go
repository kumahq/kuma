	...
	Describe("GetReachableBackends", func() {
		...
		Context("reachableBackends refs across kinds", func() {
			...
			BeforeEach(func() {
				...
				mzs = builders.MeshMultiZoneService().
					WithName("mzs").
					WithServiceLabelSelector(map[string]string{mesh_proto.DisplayName: "svc-a"}).
					AddIntPort(8080, metadata.ProtocolHTTP).
					Build()
				destinations = []core_model.Resource{msA, msB, mes, mzs}
			})

			reachable := func(refs ...*mesh_proto.Dataplane_Networking_TransparentProxying_ReachableBackendRef) (map[kri.Identifier]core.Port, bool) {
				dp := builders.Dataplane().WithAddress("127.0.0.1").Build()
				dp.Spec.Networking.TransparentProxying = &mesh_proto.Dataplane_Networking_TransparentProxying{
					ReachableBackends: &mesh_proto.Dataplane_Networking_TransparentProxying_ReachableBackends{Refs: refs},
				}
			...
			It("should select every backend of each kind", func() {
				...
				Expect(outbounds).To(HaveKey(kri.WithSectionName(kri.From(mes), "9000")))
				Expect(outbounds).To(HaveKey(kri.WithSectionName(kri.From(mzs), "8080")))
			})
