...
var _ = Describe("Dataplane Callbacks", func() {
	...
	callbacks := util_xds_v3.AdaptDeltaCallbacks(DataplaneCallbacksToXdsCallbacks(countingCallbacks))
	...
	It("should call DataplaneCallbacks correctly", func() {
		...
		err := callbacks.OnDeltaStreamOpen(context.Background(), 1, "")
		...
		err = callbacks.OnStreamDeltaRequest(1, &req)
		Expect(err).ToNot(HaveOccurred())
		...
		err = callbacks.OnDeltaStreamOpen(context.Background(), 2, "")
		Expect(err).ToNot(HaveOccurred())
		err = callbacks.OnStreamDeltaRequest(2, &req)
		...
		callbacks.OnDeltaStreamClosed(1, node)
		...
		callbacks.OnDeltaStreamClosed(2, node)
