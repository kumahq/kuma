...
// The problem
//   When you send Cluster of type EDS to Envoy, it updates the config, but Cluster is marked as warming and is not used until you send EDS
...
//
// The solution
//   The easiest solution would be to just set a new version of endpoints also when cluster changes. The problem is that go-control-plane does not support resource ordering
...
//   Nonce is a sequence indicator of a sent DiscoveryResponse on a stream. We use ADS therefore every single DiscoveryResponse regardless of a type is with incremented nonce.
//   Typical sequence of CDS + EDS looks like this:
//   1) Envoy sends DiscoveryRequest [type=CDS, version="", nonce=""]
//   2) Kuma sends DiscoveryResponse [type=CDS, version="UUID-1", nonce="1"]
//   3) Envoy sends DiscoveryRequest [type=EDS, version="", nonce=""]
//   4) Envoy sends DiscoveryRequest [type=CDS, version="UUID-1", nonce="1"] // confirmations that it received clusters (ACK)
//   5) Kuma sends DiscoveryResponse [type=EDS, version="UUID-2", nonce="2"]
//   6) Envoy sends DiscoveryRequest [type=EDS, version="UUID-2", nonce="2"] // confirmations that it received clusters (ACK)
//
//   Then if we send a Cluster update (continuing the flow above)
//   7) Kuma sends DiscoveryResponse [type=CDS, version="UUID-2", nonce="3"]
//   8) Envoy sends DiscoveryRequest [type=CDS, version="UUID-2", nonce="3"] // ACK
//   9) Envoy sends DiscoveryRequest [type=EDS, version="UUID-2", nonce="2"] // Envoy a request which looks like a second ACK for previous endpoints
//
...
//   10) Kuma sends DiscoveryResponse [type=EDS, version="UUID-3", nonce="3"] // triggered because we set snapshot with new version
...
func (r *resourceWarmingForcer) OnStreamRequest(streamID int64, request *envoy_api.DiscoveryRequest) error {
