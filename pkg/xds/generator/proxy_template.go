...
func (_ OutboundProxyGenerator) Generate(ctx xds_context.Context, proxy *model.Proxy) ([]*Resource, error) {
	...
	for _, oface := range ofaces {
		...
		if used := names[outboundListenerName]; !used {
			listener, err := envoy.CreateOutboundListener(ctx, outboundListenerName, endpoint.DataplaneIP, endpoint.DataplanePort, edsClusterName, virtual, proxy.Logs.Outbounds[oface.Interface], proxy.Metadata)
			if err != nil {
