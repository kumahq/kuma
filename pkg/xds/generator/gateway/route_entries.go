...
// GenerateEnvoyRouteEntries converts the legacy MeshGatewayRoute resources
// matched to a gateway host into route entries. This keeps
// MeshGatewayRoute-derived route information available for inspection even
// though it is no longer used to generate live Envoy config.
func GenerateEnvoyRouteEntries(host GatewayHost) []route.Entry {
	...
	for _, gatewayRoute := range gatewayRoutes {
		for _, rule := range gatewayRoute.Spec.GetConf().GetHttp().GetRules() {
			entry := makeHttpRouteEntry(gatewayRoute.GetMeta().GetName(), rule)
		...
		for _, rule := range gatewayRoute.Spec.GetConf().GetTcp().GetRules() {
			entries = append(entries,
				makeTcpRouteEntry(gatewayRoute.GetMeta().GetName(), rule),
			)
		}
	}
...
// HandlePrefixMatchesAndPopulatePolicies expands prefix path matches into a
// matching pair of prefix and exact matches, then populates the connection
// policies that apply to each route entry's destinations.
