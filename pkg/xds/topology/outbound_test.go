...
var _ = Describe("TrafficRoute", func() {
	...
	Describe("GetOutboundTargets()", func() {
		It("should pick proper dataplanes for each outbound destination", func() {
			...
			// when
			targets := BuildEdsEndpointMap(context.Background(), "zone-1", nil, nil, nil, dataplanes.Items, nil, dataSourceLoader, defaultMeshWithMTLS.MTLSEnabled(), nil)
		...
		It("should build endpoint tags entirely from dataplane labels", func() {
			...
			// when
			targets := BuildEdsEndpointMap(context.Background(), "zone-1", nil, nil, nil, dataplanes, nil, dataSourceLoader, defaultMeshWithMTLS.MTLSEnabled(), nil)
		...
		It("should prefer the inbound service tag over the dataplane service label", func() {
			...
			dp := &core_mesh.DataplaneResource{
				Meta: &test_model.ResourceMeta{
					Mesh:   "default",
					Name:   "backend-1",
					Labels: map[string]string{mesh_proto.ServiceTag: "labeled"},
				},
				Spec: &mesh_proto.Dataplane{
					Networking: &mesh_proto.Dataplane_Networking{
						Address: "192.168.0.1",
	...
	Describe("BuildEndpointMap()", func() {
		...
		DescribeTable("should include only those dataplanes that match given selectors",
			func(given testCase) {
				...
				endpoints := BuildEdsEndpointMap(context.Background(), "zone-1", given.meshServices, given.meshMultiZoneService, given.meshExternalServices, given.dataplanes, given.meshZoneAddresses, dataSourceLoader, given.mesh.MTLSEnabled(), egressAddresses)
				// then
