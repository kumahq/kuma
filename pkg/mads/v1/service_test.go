...
import (
	...
	"github.com/kumahq/kuma/v2/pkg/mads/v1/service"
...
var _ = Describe("MADS http service", func() {
	...
	Context("with resources", func() {
		...
		It("should respond with an empty discovery response", func() {
			...
			// when
			req, err := http.NewRequestWithContext(context.Background(), http.MethodPost, monitoringAssignmentPath, strings.NewReader(reqBytes))
			Expect(err).ToNot(HaveOccurred())
	...
	Context("with resources", func() {
		...
		It("should return straightaway on first request with a fetch timeout", func() {
			...
			// when
			req, err := http.NewRequestWithContext(context.Background(), http.MethodPost, monitoringAssignmentPath+"?fetch-timeout=1s", strings.NewReader(reqBytes))
			Expect(err).ToNot(HaveOccurred())
			req.Header.Add("content-type", "application/json")
		...
		It("should return no update if the fetch times out", func() {
			...
			// when
			req, err = http.NewRequestWithContext(context.Background(), http.MethodPost, monitoringAssignmentPath+"?fetch-timeout=1ms", strings.NewReader(reqBytes))
			Expect(err).ToNot(HaveOccurred())
			req.Header.Add("content-type", "application/json")
		...
		It("should allow synchronous requests", func() {
			...
			// when
			req, err := http.NewRequestWithContext(context.Background(), http.MethodPost, monitoringAssignmentPath+"?fetch-timeout=0s", strings.NewReader(reqBytes))
			Expect(err).ToNot(HaveOccurred())
			req.Header.Add("content-type", "application/json")
		...
		It("should return an error if the fetch timeout is unparseable", func() {
			...
			// when
			req, err := http.NewRequestWithContext(context.Background(), http.MethodPost, monitoringAssignmentPath+"?fetch-timeout=not-a-timeout", strings.NewReader(reqBytes))
			Expect(err).ToNot(HaveOccurred())
			req.Header.Add("content-type", "application/json")
