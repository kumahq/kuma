...
func init() {
	...
	if rawOpenAPISpec != nil {
		...
		// Extract schema from .components.schemas.MeshPassthroughItem
		...
		schemaItem, ok := schemas["MeshPassthroughItem"].(map[string]interface{})
		if !ok {
			panic(fmt.Errorf("MeshPassthroughItem schema not found"))
		}
	...
	rawOpenAPISpec = nil
	MeshPassthroughResourceTypeDescriptor.Validator = validator
...
func NewMeshPassthroughResourceList() *model.ResList[*MeshPassthroughResource] {
	return &model.ResList[*MeshPassthroughResource]{NewFn: NewMeshPassthroughResource}
}
