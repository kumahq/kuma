...
func applyToGateway(
	...
	for _, listenerInfo := range gateway_plugin.ExtractGatewayListeners(proxy) {
		...
		conf = getConf(toRules, core_rules.MeshElement())
			...
			if ok {
				for _, vh := range route.VirtualHosts {
					for _, r := range vh.Routes {
						routeConf := getConf(toRules, core_rules.MeshElement().WithKeyValue(core_rules.RuleMatchesHashTag, r.Name))
						if routeConf == nil {
							if conf == nil {
								continue
							}
		...
		for _, listenerHostnames := range listenerInfo.ListenerHostnames {
			for _, hostInfo := range listenerHostnames.HostInfos {
				...
				for _, dest := range destinations {
					...
					conf := getConf(toRules, core_rules.MeshServiceElement(serviceName))
					if conf == nil {
						continue
					}
