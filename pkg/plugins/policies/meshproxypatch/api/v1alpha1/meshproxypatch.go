...
// MeshProxyPatch provides advanced customization of the Envoy proxy configuration generated by Kuma. It allows you to add, remove, or modify Envoy resources (clusters, listeners, filters, virtual hosts) using YAML patches or JSON patches for fine-grained control beyond standard policies.
// +kuma:policy:order=9999
...
// ModOperation is modification operation on Envoy's resource. It carries no
// enum of its own: the operations a modification accepts depend on the Envoy
// resource it targets, so each field declares the subset it allows.
...
type VirtualHostMod struct {
	// Match is a set of conditions that have to be matched for modification operation to happen.
	Match *VirtualHostMatch `json:"match,omitempty"`
	// Operation to execute on matched listener.
	// +kubebuilder:validation:Enum=Add;Remove;Patch
	Operation ModOperation `json:"operation"`
	// Value of xDS resource in YAML format to add or patch.
