...
func isTargetedBy(tr *common_proto.TargetRef, dpp *core_mesh.DataplaneResource) bool {
	switch tr.GetKindEnum() {
	case common_proto.TargetRef_Mesh:
		return true
	case common_proto.TargetRef_MeshSubset:
		return isDataplaneSelectedBy(tr.GetTags(), dpp)
	case common_proto.TargetRef_MeshService:
		return isDataplaneSelectedBy(map[string]string{
			mesh_proto.ServiceTag: tr.GetName(),
		}, dpp)
	case common_proto.TargetRef_MeshServiceSubset:
		trTags := tr.GetTags()
		trTags[mesh_proto.ServiceTag] = tr.GetName()
		return isDataplaneSelectedBy(trTags, dpp)
	default:
		return false
	}
}

func isDataplaneSelectedBy(tags map[string]string, dpp *core_mesh.DataplaneResource) bool {
