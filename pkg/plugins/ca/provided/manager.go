...
func (p *providedCaManager) GenerateDataplaneCert(ctx context.Context, mesh string, backend *mesh_proto.CertificateAuthorityBackend, workload string) (ca.KeyPair, error) {
	meshCa, err := p.getCa(ctx, mesh, backend)
	if err != nil {
	...
	keyPair, err := ca_issuer.NewWorkloadCert(meshCa, mesh, workload, opts...)
	if err != nil {
		return ca.KeyPair{}, errors.Wrapf(err, "failed to generate a Workload Identity cert for workload %q in Mesh %q using backend %q", workload, mesh, backend.Name)
	}
	return *keyPair, nil
}
