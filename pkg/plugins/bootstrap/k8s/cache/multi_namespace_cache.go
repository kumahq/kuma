...
type NewCacheFunc func(config *rest.Config, opts k8s_cache.Options) (k8s_cache.Cache, error)
...
func MultiNamespacedCacheBuilder(namespaces []string) NewCacheFunc {
	return func(config *rest.Config, opts k8s_cache.Options) (k8s_cache.Cache, error) {
		opts, err := defaultOpts(config, opts)
		if err != nil {
			return nil, err
		}
		caches := map[string]k8s_cache.Cache{}
...
type multiNamespaceCache struct {
	namespaceToCache map[string]k8s_cache.Cache
	Scheme           *runtime.Scheme
}

var _ k8s_cache.Cache = &multiNamespaceCache{}

// Methods for multiNamespaceCache to conform to the Informers interface
func (c *multiNamespaceCache) GetInformer(ctx context.Context, obj runtime.Object) (k8s_cache.Informer, error) {
	informers := map[string]k8s_cache.Informer{}
...
func (c *multiNamespaceCache) GetInformerForKind(ctx context.Context, gvk schema.GroupVersionKind) (k8s_cache.Informer, error) {
	informers := map[string]k8s_cache.Informer{}
	for ns, cache := range c.namespaceToCache {
...
func (c *multiNamespaceCache) Start(stopCh <-chan struct{}) error {
	for ns, cache := range c.namespaceToCache {
		go func(ns string, cache k8s_cache.Cache) {
...
type multiNamespaceInformer struct {
	namespaceToInformer map[string]k8s_cache.Informer
}

var _ k8s_cache.Informer = &multiNamespaceInformer{}
