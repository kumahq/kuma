...
func regexOf(regex string) *envoy_type_matcher_v3.RegexMatcher {
...
func RouteMirror(percent float64, destination Destination) RouteConfigurer {
	...
	return RouteConfigureFunc(func(r *envoy_config_route_v3.Route) error {
		...
		action.RequestMirrorPolicies = append(action.RequestMirrorPolicies,
			&envoy_config_route_v3.RouteAction_RequestMirrorPolicy{
				Cluster: clusterName,
...
func RouteActionRedirect(redirect *Redirection) RouteConfigurer {
	...
	return RouteConfigureFunc(func(r *envoy_config_route_v3.Route) error {
		r.Action = &envoy_config_route_v3.Route_Redirect{
		...
		case 301:
			r.GetRedirect().ResponseCode = envoy_config_route_v3.RedirectAction_MOVED_PERMANENTLY
		case 302:
			r.GetRedirect().ResponseCode = envoy_config_route_v3.RedirectAction_FOUND
		case 303:
			r.GetRedirect().ResponseCode = envoy_config_route_v3.RedirectAction_SEE_OTHER
		case 307:
			r.GetRedirect().ResponseCode = envoy_config_route_v3.RedirectAction_TEMPORARY_REDIRECT
		case 308:
			r.GetRedirect().ResponseCode = envoy_config_route_v3.RedirectAction_PERMANENT_REDIRECT
		default:
...
func RouteActionForward(destinations []Destination) RouteConfigurer {
	...
	return RouteConfigureFunc(func(r *envoy_config_route_v3.Route) error {
		...
		var total uint32
		var weights []*envoy_config_route_v3.WeightedCluster_ClusterWeight

		for n, d := range byName {
			total += d.Weight
			weights = append(weights, &envoy_config_route_v3.WeightedCluster_ClusterWeight{
				Name:   n,
				Weight: util_proto.UInt32(d.Weight),
			})
		}

		r.Action = &envoy_config_route_v3.Route_Route{
...
// route builders should be configured onn virtual hosts in the intended
// match order.
func VirtualHostRoute(route *RouteBuilder) envoy_routes.VirtualHostBuilderOpt {
	return envoy_routes.AddVirtualHostConfigurer(
		v3.VirtualHostConfigureFunc(func(vh *envoy_config_route_v3.VirtualHost) error {
