...
var _ = Describe("Gateway Listener", func() {
	...
	BeforeEach(func() {
		...
		Expect(StoreNamedFixture(rt, "traffic-route-default.yaml")).To(Succeed())
		...
		Expect(StoreNamedFixture(rt, "traffic-route-tracing.yaml")).To(Succeed())
		...
		Expect(StoreNamedFixture(rt, "traffic-route-logging.yaml")).To(Succeed())
