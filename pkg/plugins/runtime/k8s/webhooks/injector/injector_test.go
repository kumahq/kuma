  ...
  initPatch:
	...
	DescribeTableSubtree("should inject Kuma into a Pod",
		func(given testCase) {
			...
			run := func(sidecarsEnabled bool) {
				...
				injector, err := inject.New(cfg, "http://kuma-control-plane.kuma-system:5681", k8sClient, sidecarsEnabled, k8s.NewSimpleConverter(), 9901, false, systemNamespace, nil)
				Expect(err).ToNot(HaveOccurred())
	...
	DescribeTable("should not inject Kuma into a Pod",
		func(given testCase) {
			...
			Expect(config.Load(filepath.Join("testdata", given.cfgFile), &cfg)).To(Succeed())
			cfg.CaCertFile = caCertPath
			injector, err := inject.New(cfg, "http://kuma-control-plane.kuma-system:5681", k8sClient, false, k8s.NewSimpleConverter(), 9901, false, systemNamespace, nil)
			Expect(err).ToNot(HaveOccurred())
