...
var _ = Describe("Migrate", func() {
	...
	It("should migrate DB", func() {
		...
		// then
		Expect(err).ToNot(HaveOccurred())
		Expect(ver).To(Equal(plugins.DbVersion(1604501001)))
	...
	It("should throw an error when trying to run migrations on newer migration version of DB than in Kuma", func() {
		...
		// then
		Expect(err).To(MatchError("DB is migrated to newer version than Kuma. DB migration version 9999999999. Kuma migration version 1604501001. Run newer version of Kuma"))
	})
