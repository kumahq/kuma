...
var _ = Describe("newMetaAdapter", func() {
	DescribeTable("namespace label",
		func(namespace string, stored map[string]string, expected string) {
			obj := &meshtimeout_k8s.MeshTimeout{
				Name: "res-1", Namespace: namespace, Labels: stored,
				Spec: &meshtimeout_api.MeshTimeout{TargetRef: &common_api.TargetRef{Kind: common_api.Mesh}},
			}
	...
	It("should not add the namespace label to an object without a namespace", func() {
		obj := &meshtimeout_k8s.MeshTimeout{
			Name: "default",
			Spec: &meshtimeout_api.MeshTimeout{TargetRef: &common_api.TargetRef{Kind: common_api.Mesh}},
		}
