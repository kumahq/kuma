...
func buildKumaNetConfig(tpConfig *config.TransparentProxyConfig) (*kumanet_config.Config, error) {
	redirectInboundPort, err := parseUint16(tpConfig.RedirectPortInBound)
	if err != nil {
		return nil, errors.Wrap(err, "parsing inbound redirect port failed")
	}
	...
	if tpConfig.RedirectPortInBoundV6 != "" {
		...
		if err != nil {
			return nil, errors.Wrap(err, "parsing inbound redirect port IPv6 failed")
		}
	}

	redirectOutboundPort, err := parseUint16(tpConfig.RedirectPortOutBound)
	if err != nil {
		return nil, errors.Wrap(err, "parsing outbound redirect port failed")
	}

	agentDNSListenerPort, err := parseUint16(tpConfig.AgentDNSListenerPort)
	if err != nil {
		return nil, errors.Wrap(err, "parsing agent DNS listener port failed")
	}
	...
	if tpConfig.ExcludeInboundPorts != "" {
		...
		if err != nil {
			return nil, errors.Wrap(err, "cannot parse inbound ports to exclude")
		}
	}
	...
	if len(tpConfig.ExcludeOutboundTCPPortsForUIDs) > 0 {
		...
		if err != nil {
			return nil, errors.Wrap(err, "parsing excluded outbound TCP ports for UIDs failed")
		}
	...
	if len(tpConfig.ExcludeOutboundUDPPortsForUIDs) > 0 {
		...
		if err != nil {
			return nil, errors.Wrap(err, "parsing excluded outbound UDP ports for UIDs failed")
		}
	...
	if tpConfig.ExcludeOutboundPorts != "" {
		...
		if err != nil {
			return nil, errors.Wrap(err, "cannot parse outbound ports to exclude")
		}
	}

	ipv6, err := ShouldEnableIPv6()
	if err != nil {
		return nil, errors.Wrap(err, "cannot verify if IPv6 should be enabled")
	}

	return &kumanet_config.Config{
