...
var _ = Describe("Updater", func() {
	...
	It("should not initialize MeshIdentity if meshService.mode is not Exclusive", func() {
		...
		Expect(
			samples.MeshDefaultBuilder().
				WithMeshServicesEnabled(mesh_proto.Mesh_MeshServices_Disabled).
				Create(resManager),
		).To(Succeed())
	...
	It("should update the trust domain of an existing meshtrust when the zone is renamed", func() {
		// when a MeshTrust was created while the zone still had its old name,
		// which is what happens to a standalone zone right before it is federated
		Expect(
			samples.MeshDefaultBuilder().
				WithMeshServicesEnabled(mesh_proto.Mesh_MeshServices_Exclusive).
				Create(resManager),
		...
		// then the reconciler moves it to the trust domain the provider now issues
		// certificates in, otherwise Envoy has no bundle for the presented SPIFFE ID
		Eventually(func(g Gomega) {
			trust := meshtrust_api.NewMeshTrustResource()
