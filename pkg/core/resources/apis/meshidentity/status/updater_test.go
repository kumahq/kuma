...
var _ = Describe("Updater", func() {
	...
	It("should set status.origin when creating MeshTrust", func() {
		// given - create mesh
		Expect(
			samples.MeshDefaultBuilder().
				WithMeshServicesEnabled(mesh_proto.Mesh_MeshServices_Exclusive).
		...
		// when - create MeshIdentity which triggers MeshTrust creation
		identity := builders.MeshIdentity().WithBundledAutoGenerated().WithName("identity").Build()
		Expect(resManager.Create(context.Background(), identity, store.CreateBy(model.MetaToResourceKey(identity.GetMeta())))).To(Succeed())

		// then - status.origin should be set by the updater
		Eventually(func(g Gomega) {
			trust := meshtrust_api.NewMeshTrustResource()
			err := resManager.Get(context.Background(), trust, store.GetByKey("identity", "default"))
			g.Expect(err).ToNot(HaveOccurred())

			// Status.origin should be set by the updater
			g.Expect(trust.Status).ToNot(BeNil())
			g.Expect(trust.Status.Origin).ToNot(BeNil())
