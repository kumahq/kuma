...
var _ = Describe("Updater", func() {
	...
	It("should pin the rendered trust domain in the status", func() {
		// when
		Expect(
			samples.MeshDefaultBuilder().
				Create(resManager),
		...
		Eventually(func(g Gomega) {
			mid := meshidentity_api.NewMeshIdentityResource()
			g.Expect(resManager.Get(context.Background(), mid, store.GetByKey("identity", "default"))).To(Succeed())
			g.Expect(pointer.Deref(mid.Status.TrustDomain)).To(Equal("default.east.mesh.local"))
		}, "10s", "100ms").Should(Succeed())
	})

	It("should move an existing meshtrust onto the trust domain it pins", func() {
		// the MeshTrust was written while the zone still had its old name, which is
		// what a standalone zone looks like right before it is federated, and the
		// identity has never been pinned. Leaving the MeshTrust behind would key the
		// CA bundle under a domain no leaf presents
		Expect(
			samples.MeshDefaultBuilder().
				Create(resManager),
		).To(Succeed())
		meshTrust := builders.MeshTrust().
			WithName("identity").
			WithTrustDomain("default.default.mesh.local").
			Build()
		Expect(resManager.Create(context.Background(), meshTrust, store.CreateBy(model.MetaToResourceKey(meshTrust.GetMeta())))).To(Succeed())
		...
		Eventually(func(g Gomega) {
			trust := meshtrust_api.NewMeshTrustResource()
			g.Expect(resManager.Get(context.Background(), trust, store.GetByKey("identity", "default"))).To(Succeed())
			g.Expect(trust.Spec.TrustDomain).To(Equal("default.east.mesh.local"))
	...
	It("should keep the pinned trust domain when the zone is renamed", func() {
		// the identity was pinned while the zone was still called "default" and its
		// MeshTrust agrees. The reconciler now runs as zone "east", so without the pin
		// both would re-render and every already issued leaf would stop verifying
		Expect(
			samples.MeshDefaultBuilder().
				Create(resManager),
		).To(Succeed())
		...
		identity := builders.MeshIdentity().WithBundledAutoGenerated().WithName("identity").Build()
		identity.Status = &meshidentity_api.MeshIdentityStatus{TrustDomain: pointer.To("default.default.mesh.local")}
		Expect(resManager.Create(context.Background(), identity, store.CreateBy(model.MetaToResourceKey(identity.GetMeta())))).To(Succeed())
		...
		Eventually(func(g Gomega) {
			mid := meshidentity_api.NewMeshIdentityResource()
			g.Expect(resManager.Get(context.Background(), mid, store.GetByKey("identity", "default"))).To(Succeed())
			g.Expect(mid.Status.IsInitialized()).To(BeTrue())
		}, "10s", "100ms").Should(Succeed())
		...
		Consistently(func(g Gomega) {
			trust := meshtrust_api.NewMeshTrustResource()
			g.Expect(resManager.Get(context.Background(), trust, store.GetByKey("identity", "default"))).To(Succeed())
			g.Expect(trust.Spec.TrustDomain).To(Equal("default.default.mesh.local"))

			mid := meshidentity_api.NewMeshIdentityResource()
			g.Expect(resManager.Get(context.Background(), mid, store.GetByKey("identity", "default"))).To(Succeed())
			g.Expect(pointer.Deref(mid.Status.TrustDomain)).To(Equal("default.default.mesh.local"))
