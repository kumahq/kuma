...
var _ = Describe("Bundled Providers Test", func() {
	...
	BeforeEach(func() {
		...
		bundledProvider, err = bundled.NewBundledIdentityProvider(secretManager, secretManager, metrics, "my-zone", config_core.KubernetesEnvironment)
		Expect(err).ToNot(HaveOccurred())
	})
	...
	Context("CreateIdentity", func() {
		It("should provide a identity for a dataplane in Kubernetes", func() {
			meshIdentity := builders.MeshIdentity().
				WithName("matching-1").
				WithSelector(&v1alpha1.LabelSelector{
					MatchLabels: &map[string]string{
		...
		It("should provide a identity for a dataplane in Universal", func() {
			// given - universal environment provider
			universalMetrics, err := core_metrics.NewMetrics("universal")
			Expect(err).ToNot(HaveOccurred())
			universalProvider, err := bundled.NewBundledIdentityProvider(secretManager, secretManager, universalMetrics, "my-zone", config_core.UniversalEnvironment)
			Expect(err).ToNot(HaveOccurred())

			meshIdentity := builders.MeshIdentity().
				WithName("matching-universal").
				WithSelector(&v1alpha1.LabelSelector{
					MatchLabels: &map[string]string{
						"app": "test-app",
					},
			...
			// when
			err = universalProvider.Initialize(context.TODO(), meshIdentity)
			Expect(err).ToNot(HaveOccurred())
