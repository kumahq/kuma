...
func (g *Generator) meshServicesForDataplane(dataplane *core_mesh.DataplaneResource) map[string]*meshservice_api.MeshService {
	...
	for _, inbound := range dataplane.Spec.GetNetworking().GetInbound() {
		...
		if !core_mesh.DNS1035LabelRegexp.MatchString(serviceTagValue) {
			log.Info("couldn't generate MeshService from kuma.io/service, contains invalid characters", "value", serviceTagValue, "regex", core_mesh.DNS1035LabelRegexp)
			continue
		}
