...
var _ = Describe("StatusUpdater", func() {
	...
	createMOTB := func(name string, opts ...store.CreateOptionsFunc) {
	...
	getConditions := func(name string) func() ([]common_api.Condition, error) {
		return func() ([]common_api.Condition, error) {
			motb := motb_api.NewMeshOpenTelemetryBackendResource()
			err := resManager.Get(context.Background(), motb, store.GetByKey(name, core_model.DefaultMesh))
			if err != nil {
				return nil, err
			}
	...
	getMeshMetricConditions := func(name string) func() ([]common_api.Condition, error) {
		return func() ([]common_api.Condition, error) {
			...
			err := resManager.Get(context.Background(), mm, store.GetByKey(name, core_model.DefaultMesh))
			if err != nil {
				return nil, err
			}
			if mm.Status == nil {
	...
	getMeshTraceConditions := func(name string) func() ([]common_api.Condition, error) {
		return func() ([]common_api.Condition, error) {
			...
			err := resManager.Get(context.Background(), mt, store.GetByKey(name, core_model.DefaultMesh))
			if err != nil {
				return nil, err
			}
			if mt.Status == nil {
	...
	getMeshAccessLogConditions := func(name string) func() ([]common_api.Condition, error) {
		return func() ([]common_api.Condition, error) {
			...
			err := resManager.Get(context.Background(), mal, store.GetByKey(name, core_model.DefaultMesh))
			if err != nil {
				return nil, err
			}
			if mal.Status == nil {
	...
	It("should set Referenced condition when MeshMetric references the backend", func() {
		...
		Expect(resManager.Create(context.Background(), mm, store.CreateByKey("mm-1", core_model.DefaultMesh))).To(Succeed())
	...
	It("should set Referenced condition when MeshTrace references the backend", func() {
		...
		Expect(resManager.Create(context.Background(), mt, store.CreateByKey("mt-1", core_model.DefaultMesh))).To(Succeed())
	...
	It("should set Referenced condition when MeshAccessLog references the backend", func() {
		...
		Expect(resManager.Create(context.Background(), mal, store.CreateByKey("mal-1", core_model.DefaultMesh))).To(Succeed())
	...
	It("should change condition when references are removed", func() {
		...
		Expect(resManager.Delete(context.Background(), meshmetric_api.NewMeshMetricResource(), store.DeleteByKey("mm-1", core_model.DefaultMesh))).To(Succeed())
	...
	It("should mark MeshMetric backendRefs unresolved when MOTB is missing", func() {
		...
		Expect(resManager.Create(context.Background(), mm, store.CreateByKey("mm-missing", core_model.DefaultMesh))).To(Succeed())
	...
	It("should mark MeshTrace backendRefs unresolved when MOTB is missing", func() {
		...
		Expect(resManager.Create(context.Background(), mt, store.CreateByKey("mt-missing", core_model.DefaultMesh))).To(Succeed())
	...
	It("should mark MeshAccessLog backendRefs unresolved when MOTB is missing", func() {
		...
		Expect(resManager.Create(context.Background(), mal, store.CreateByKey("mal-missing", core_model.DefaultMesh))).To(Succeed())
	...
	It("should flip MeshMetric backendRefs condition to resolved when backend appears", func() {
		...
		Expect(resManager.Create(context.Background(), mm, store.CreateByKey("mm-appearing", core_model.DefaultMesh))).To(Succeed())
		...
		Eventually(getMeshMetricConditions("mm-appearing"), "10s", "100ms").Should(ContainElement(common_api.Condition{
			Type:    common_api.BackendRefsResolvedCondition,
			Status:  kube_meta.ConditionTrue,
			Reason:  common_api.AllBackendRefsResolvedReason,
			Message: "All MeshOpenTelemetryBackend references are resolved",
		}))
	})

	It("should resolve label-based backendRef and count as referenced", func() {
		createMOTB("collector-abc123",
			store.CreateWithLabels(map[string]string{"app": "otel-collector", "env": "prod"}),
		)
		...
		Expect(resManager.Create(context.Background(), mm, store.CreateByKey("mm-labels", core_model.DefaultMesh))).To(Succeed())

		Eventually(getConditions("collector-abc123"), "10s", "100ms").Should(ContainElement(common_api.Condition{
			Type:    motb_api.ReferencedByPoliciesCondition,
			Status:  kube_meta.ConditionTrue,
			Reason:  motb_api.ReferencedReason,
			Message: "Referenced by 1 policy backend(s)",
		...
		Eventually(getMeshMetricConditions("mm-labels"), "10s", "100ms").Should(ContainElement(common_api.Condition{
			Type:    common_api.BackendRefsResolvedCondition,
			Status:  kube_meta.ConditionTrue,
			Reason:  common_api.AllBackendRefsResolvedReason,
			Message: "All MeshOpenTelemetryBackend references are resolved",
	...
	It("should report unresolved when label-based backendRef matches no backend", func() {
		mm := meshmetric_api.NewMeshMetricResource()
		mm.Spec = &meshmetric_api.MeshMetric{
			Default: meshmetric_api.Conf{
				Backends: &[]meshmetric_api.Backend{
		...
		Expect(resManager.Create(context.Background(), mm, store.CreateByKey("mm-unresolved-labels", core_model.DefaultMesh))).To(Succeed())

		Eventually(getMeshMetricConditions("mm-unresolved-labels"), "10s", "100ms").Should(ContainElement(common_api.Condition{
			Type:    common_api.BackendRefsResolvedCondition,
			Status:  kube_meta.ConditionFalse,
			Reason:  common_api.UnresolvedBackendRefsReason,
			Message: "Unresolved MeshOpenTelemetryBackend references: labels:app=nonexistent",
		}))
	})
