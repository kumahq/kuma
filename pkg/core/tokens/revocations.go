...
func (s *secretRevocations) IsRevoked(id string) (bool, error) {
	data, err := s.getSecretData()
	if err != nil {
		return false, err
	}
	if len(data) == 0 {
...
func (s *secretRevocations) getSecretData() ([]byte, error) {
	...
	if err := s.manager.Get(context.Background(), resource, core_store.GetBy(s.revocationKey)); err != nil {
