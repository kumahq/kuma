...
demo/dp-sever: # start dp-server
	$(KUMA_DIR)/demo/grpc/server/start_grpc_server_dp.sh


.PHONY: demo/grpc-sever
demo/grpc-sever: # start grpc-server
	$(KUMA_DIR)/build/artifacts-darwin-amd64/test-server/test-server grpc server --ip ${ip} --port 2345


.PHONY: demo/dp-client
demo/dp-client: # start dp-client
	$(KUMA_DIR)/demo/grpc/client/start_grpc_client_dp.sh


.PHONY: demo/grpc-client
demo/grpc-client: # start grpc-client
	$(KUMA_DIR)/build/artifacts-darwin-amd64/test-server/test-server grpc client --address 127.0.0.1:8989
