...
ifeq ($(KUMA_STORE_TYPE),postgres)
	...
	$(CP_ENV) $(DISTRIBUTION_FOLDER)/bin/kuma-cp run --log-level=info
...
run/kuma-dp: $(DISTRIBUTION_FOLDER) ## Dev: Run `kuma-dp` locally
	...
	$(DISTRIBUTION_FOLDER)/bin/kuma-dp run --log-level=info --dataplane-token-file /tmp/kuma-dp-$(EXAMPLE_DATAPLANE_NAME)-$(EXAMPLE_DATAPLANE_MESH)-token --dataplane-file /tmp/kuma-dp-$(EXAMPLE_DATAPLANE_NAME)-$(EXAMPLE_DATAPLANE_MESH).yaml
