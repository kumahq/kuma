...
var _ = Describe("Zone Egress Insights", func() {
	Context("UpdateSubscription", func() {
		...
		It("should return error for wrong subscription type", func() {
			...
			Expect(err.Error()).To(Equal("invalid type *v1alpha1.KDSSubscription for ZoneEgressInsight"))
		})
	})
})
