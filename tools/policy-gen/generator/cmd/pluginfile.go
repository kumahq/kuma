		...
		RunE: func(cmd *cobra.Command, _ []string) error {
			...
			if pconfig.IsPolicy {
				for _, version := range versions {
					orderOutPath := filepath.Join(rootArgs.pluginDir, "plugin", version, "zz_generated.order.go")
					if err := commontemplate.GoTemplate(pluginOrderTemplate, struct {
						Version string
						Order   int
					}{
						Version: version,
						Order:   pconfig.Order,
					}, orderOutPath); err != nil {
						return err
					}
				}
			}
...
var pluginOrderTemplate = template.Must(template.New("plugin-order").Parse(`
