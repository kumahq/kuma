...
import (
	...
	"github.com/kumahq/kuma/v3/pkg/plugins/authn/api-server/tokens/ws/client"
...
var _ = Describe("Auth Tokens WS", func() {
	...
	It("should throw an error if validFor is not present", func() {
		...
		req, err := http.NewRequestWithContext(context.Background(), http.MethodPost, "/tokens/user", strings.NewReader(`{"name": "xyz"}`))
