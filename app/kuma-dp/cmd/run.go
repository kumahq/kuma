...
func newRunCmd(opts kuma_cmd.RunCmdOpts, rootCtx *RootContext) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		PostRunE: func(cmd *cobra.Command, _ []string) error {
			...
			bootstrap, kumaSidecarConfiguration, err := rootCtx.BootstrapGenerator(gracefulCtx, opts.Config.ControlPlane.URL, opts.Config, envoy.BootstrapParams{
				...
				MetricsCertPath:     cfg.DataplaneRuntime.MetricsCertPath,
				MetricsKeyPath:      cfg.DataplaneRuntime.MetricsKeyPath,
			})
			if err != nil {
	...
	cmd.PersistentFlags().StringVar(&cfg.DataplaneRuntime.MetricsCertPath, "metrics-cert-path", cfg.DataplaneRuntime.MetricsCertPath, "A path to the certificate for metrics listener")
	cmd.PersistentFlags().StringVar(&cfg.DataplaneRuntime.MetricsKeyPath, "metrics-key-path", cfg.DataplaneRuntime.MetricsKeyPath, "A path to the certificate key for metrics listener")
