...
func (s *staticDataplaneTokenGenerator) Generate(name string, mesh string, tags map[string][]string, dpType string, workload string, validFor time.Duration) (string, error) {
	if s.err != nil {
		return "", s.err
	}
...
var _ = Describe("kumactl generate dataplane-token", func() {
	...
	DescribeTable("should generate token",
		...
		Entry("for all arguments", testCase{
			args:   []string{"generate", "dataplane-token", "--mesh=demo", "--name=example", "--proxy-type=dataplane", "--tag", "kuma.io/display-name=web", "--valid-for", "30s"},
			result: "token-for-example-demo-kuma.io/display-name=web-dataplane",
		}),
	)
