...
type DataplaneOverviewClient interface {
	List(ctx context.Context, meshName string, tags map[string]string, gatewayDataPlanesFilter bool) (*mesh.DataplaneOverviewResourceList, error)
}
...
func (d *httpDataplaneOverviewClient) List(ctx context.Context, meshName string, tags map[string]string, gatewayDataPlanesFilter bool) (*mesh.DataplaneOverviewResourceList, error) {
	resUrl, err := constructUrl(meshName, tags, gatewayDataPlanesFilter)
	if err != nil {
...
func constructUrl(meshName string, tags map[string]string, gatewayDataPlanesFilter bool) (*url.URL, error) {
	result, err := url.Parse(fmt.Sprintf("/meshes/%s/dataplanes+insights", meshName))
	if err != nil {
		return nil, err
	}
	query := result.Query()
	if gatewayDataPlanesFilter {
		query.Add("gateway", fmt.Sprintf("%t", gatewayDataPlanesFilter))
	}
