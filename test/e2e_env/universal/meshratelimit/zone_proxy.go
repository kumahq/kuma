...
func ZoneProxy() {
	const meshName = "mrl-zone-proxy"
	const demoClient = "demo-client"
	const egressWorkload = "zone-proxy-egress"
	const externalServiceApp = "mrl-zone-proxy-ext"
	...
	trafficAllowed := func(address string, mustPassRepeatedly int) {
		...
		if mustPassRepeatedly > 0 {
			eventually = eventually.MustPassRepeatedly(mustPassRepeatedly)
		}
		eventually.Should(Succeed())
	}

	trafficRateLimited := func(address string) {
		GinkgoHelper()
		...
		Eventually(func(g Gomega) {
			...
			g.Expect(err).ToNot(HaveOccurred())
			g.Expect(failure.ResponseCode).To(Equal(429))
	...
	It("rate limits only the MeshExternalService selected by SNI on zone egress", func() {
		...
		trafficAllowed(targetedURL, 0)
		trafficAllowed(untargetedURL, 0)
		...
		trafficRateLimited(targetedURL)
		trafficAllowed(untargetedURL, 3)
	})
}
