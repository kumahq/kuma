...
mtls:
  ...
  - name: ca-1
	...
	runDemoClient := func(mesh string) {
		Expect(
			env.Cluster.Install(DemoClientUniversal("demo-client", mesh, WithTransparentProxy(true))),
		).To(Succeed())
	}
	...
	runTestServer := func(mesh string, tls bool) {
		...
		Expect(env.Cluster.Install(TestServerUniversal("test-server", mesh, WithArgs(args), WithProtocol("tcp")))).To(Succeed())
	}
	...
	DescribeTable("should support mTLS",
		func(meshName string, yaml string) {
			err := NewClusterSetup().
...
`,
		...
		Entry("Provided CA with Root CA", "mtls-provided", `
type: Mesh
name: mtls-provided
mtls:
...
`),
		Entry("Provided CA with Intermediate CA", "mtls-provided-intermediate", `
type: Mesh
name: mtls-provided-intermediate
mtls:
