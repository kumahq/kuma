...
`
		...
		Eventually(func(g Gomega) {
			...
			g.Expect(err).ToNot(HaveOccurred())
			g.Expect(stdout).To(ContainSubstring(fmt.Sprintf("cluster.%s_backend___msvc_80.ssl.handshake", meshName)))
			g.Expect(stdout).ToNot(ContainSubstring(fmt.Sprintf("cluster.%s_backend___msvc_80.ssl.handshake: 0", meshName)))
