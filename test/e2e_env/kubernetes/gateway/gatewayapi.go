...
`, gatewayName, namespace, meshName)
		...
		BeforeAll(func() {
			...
			Expect(YamlK8s(haGatewayClass)(env.Cluster)).To(Succeed())
...
`, namespace, meshName, gatewayName)
		...
		It("should manage Kuma Secret", func() {
			...
			Eventually(func(g Gomega) {
				out, err := env.Cluster.GetKumactlOptions().RunKumactlAndGetOutput("get", "secret", "-m", meshName, convertedSecretName, "-o", "json")
				g.Expect(err).ToNot(HaveOccurred())
				kumaSecret = out
			}, "30s", "1s").Should(Succeed())
...
`, namespace)
			err := YamlK8s(secret)(env.Cluster)
			...
			err = k8s.RunKubectlE(env.Cluster.GetTesting(), env.Cluster.GetKubectlOptions(namespace), "delete", "secret", "secret-tls")

			// then copied secret is removed
			Expect(err).ToNot(HaveOccurred())
			Eventually(func(g Gomega) {
				_, err := env.Cluster.GetKumactlOptions().RunKumactlAndGetOutput("get", "secret", "-m", meshName, convertedSecretName)
...
`, namespace, meshName)
			...
			err := k8s.KubectlApplyFromStringE(env.Cluster.GetTesting(), env.Cluster.GetKubectlOptions(), gateway)
