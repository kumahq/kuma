...
func Sync() {
	...
	identityName := "meshservice-identity"
	...
	BeforeAll(func() {
		Expect(NewClusterSetup().
			...
			Install(MeshTrafficPermissionAllowAllUniversalWorkloadIdentity(
				meshName,
				MeshIdentityTrustDomains(meshName, multizone.UniZone1, multizone.UniZone2, multizone.KubeZone2)...,
			)).
...
`
	...
	It("should sync MeshService to global with VIP status", func() {
		...
		// The MeshIdentity is Bundled, so the SPIFFE ID carries the trust domain
		...
		Eventually(func(g Gomega) {
			...
			g.Expect(spec.Identities).To(Equal(expectedIdentities))
