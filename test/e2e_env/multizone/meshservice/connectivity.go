...
`, Config.KumaNamespace, meshName, namespace)
	...
	type testCase struct {
		...
		// timeout overrides the default 30s Eventually budget for
		// entries that race propagation of dynamically-created zones.
		timeout string
	}

	timeoutOrDefault := func(t string) string {
		if t == "" {
			return "30s"
		}
		return t
	...
	DescribeTable("client from Kubernetes",
		func(given testCase) {
			...
			Eventually(func(g Gomega) {
				...
				g.Expect(err).ToNot(HaveOccurred())
				g.Expect(response.Instance).To(Equal(given.expectedInstance))
			}, timeoutOrDefault(given.timeout), "1s").Should(given.should)
		},
		...
		Entry("should access service in a Universal cluster where MeshService is autogenerated", testCase{
			...
			// autogenerate-universal is a fresh zone CP created in
			// BeforeAll. Its MeshService propagation chain
			// (universal-zone MS generator 2s tick → KDS to Global
			// → KDS to KubeZone1 → envoy CDS/EDS push) can exceed
			// 30s under CI load even after WaitForZoneOnline.
			timeout: "2m",
		}),
	)
	...
	DescribeTable("client from Universal",
		...
		Entry("should access service in another Universal cluster where MeshService is autogenerated", testCase{
			address:          func() string { return "http://test-server.svc.autogenerate-universal.mesh.local:80" },
			expectedInstance: "auto-uni-test-server",
			// See note on the matching entry in "client from Kubernetes".
			timeout: "2m",
		}),
	)
}
