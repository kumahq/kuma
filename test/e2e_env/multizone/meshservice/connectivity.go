...
func Connectivity() {
	...
	autoGeneratedMeshServiceName := hash.HashedName(meshName, "test-server", autoGenerateUniversalClusterName)
...
`, Config.KumaNamespace, meshName, namespace)
		...
		kubeZone2Setup := NewClusterSetup().
			Install(NamespaceWithSidecarInjection(namespace)).
		...
		autoGenerateUniversalCluster.WithRetries(60)
		multizone.KubeZone2.WithRetries(60)
		NewClusterSetup().
			Install(Kuma(
				core.Zone,
		...
		Expect(kubeZone2Setup.SetupWithRetries(multizone.KubeZone2, 2)).To(Succeed())
		...
		Eventually(func(g Gomega) {
			_, _, err := GetMeshServiceStatus(multizone.Global, autoGeneratedMeshServiceName, meshName)
			g.Expect(err).ToNot(HaveOccurred())
		}, "2m", "2s").Should(Succeed())

		Expect(multizone.KubeZone1.WaitApp("demo-client", namespace, 1)).To(Succeed())
		Expect(multizone.KubeZone2.WaitApp("test-server", namespace, 1)).To(Succeed())
