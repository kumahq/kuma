...
`, Config.KumaNamespace, meshName, namespace)
		...
		case K3dK8sType, K3dCalicoK8sType:
			Expect(multizone.KubeZone2.LoadImages(Config.KumaUniversalImageRepo)).To(Succeed())
		}
		...
		Eventually(func(g Gomega) {
			response, err := client.CollectEchoResponse(multizone.KubeZone1, "demo-client",
				"http://test-server.msconnectivity.svc.kuma-2.mesh.local:80",
	...
	timeoutOrDefault := func(timeout string) string {
		if timeout != "" {
			return timeout
		}
		return "30s"
