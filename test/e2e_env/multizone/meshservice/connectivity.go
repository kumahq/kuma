...
spec:
  template: '{{ .DisplayName }}.{{ .Namespace }}.{{ .Zone }}.mesh-specific.k8s.msconnectivity'
  selector:
    meshService:
      matchLabels:
        kuma.io/managed-by: k8s-controller
...
spec:
  template: '{{ .DisplayName }}.{{ .Namespace }}.{{ .Zone }}.not-my-mesh-specific.k8s.msconnectivity'
  selector:
    meshService:
      matchLabels:
        kuma.io/managed-by: k8s-controller
...
spec:
  template: '{{ .DisplayName }}.{{ .Zone }}.mesh-specific.universal.msconnectivity'
  selector:
    meshService:
      matchLabels:
        kuma.io/mesh: "%s"
...
spec:
  template: '{{ .DisplayName }}.{{ .Zone }}.not-my-mesh-specific.universal.msconnectivity'
  selector:
    meshService:
      matchLabels:
        kuma.io/mesh: non-existent-mesh
...
`
	...
	DescribeTable("client from Kubernetes",
		...
		Entry("should access service in the same Kubernetes cluster via a mesh-targeted generator name", testCase{
			address:          func() string { return "http://test-server.msconnectivity.kuma-1.mesh-specific.k8s.msconnectivity:80" },
			expectedInstance: "kube-test-server-1",
		}),
		...
		Entry("should access service in another Universal cluster via a mesh-targeted generator name", testCase{
			address:          func() string { return "http://test-server.kuma-5.mesh-specific.universal.msconnectivity:80" },
			expectedInstance: "uni-test-server",
		}),
		Entry("should not be able to access service in another Universal cluster if the hostname generator for that name doesn't match", testCase{
			address:          func() string { return "http://test-server.kuma-5.not-my-mesh-specific.universal.msconnectivity:80" },
			expectedInstance: "uni-test-server",
			should:           Not(Succeed()),
		}),
	...
	DescribeTable("client from Universal",
		...
		Entry("should access service in a Kubernetes cluster via a mesh-targeted generator name", testCase{
			address:          func() string { return "http://test-server.msconnectivity.kuma-2.mesh-specific.k8s.msconnectivity:80" },
			expectedInstance: "kube-test-server-2",
		}),
		Entry("should not be able to access service in a Kubernetes cluster if the hostname generator for that name doesn't match", testCase{
			address: func() string {
				return "http://test-server.msconnectivity.kuma-2.not-my-mesh-specific.k8s.msconnectivity:80"
			},
			expectedInstance: "kube-test-server-2",
			should:           Not(Succeed()),
		}),
		Entry("should NOT YET access service in the same Universal cluster via a mesh-targeted generator name", testCase{
			address:          func() string { return "http://test-server.kuma-4.mesh-specific.universal.msconnectivity:80" },
