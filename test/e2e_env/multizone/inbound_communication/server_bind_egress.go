...
func ServerBindEgress() {
	...
	BeforeAll(func() {
		...
		Expect(WaitForMesh(meshEgress, env.Zones())).To(Succeed())
	...
	E2EAfterAll(func() {
		Expect(env.KubeZone1.TriggerDeleteNamespace(namespaceEgress)).To(Succeed())
		Expect(env.KubeZone1.DeleteMesh(meshEgress))
		Expect(env.UniZone1.DeleteMeshApps(meshEgress)).To(Succeed())
		Expect(env.Global.DeleteMeshApps(meshEgress)).To(Succeed())
	})
