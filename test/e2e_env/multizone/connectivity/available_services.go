...
func AvailableServices() {
	...
	BeforeAll(func() {
		...
		err = NewClusterSetup().
			...
			Install(MultipleIngressUniversal(ingress1Port, fmt.Sprint("%s", fmt.Sprintf("%s-%d", AppIngress, ingress1Port)), multizone.Global.GetKuma().GenerateZoneIngressToken)).
			Install(MultipleIngressUniversal(ingress2Port, fmt.Sprint("%s", fmt.Sprintf("%s-%d", AppIngress, ingress2Port)), multizone.Global.GetKuma().GenerateZoneIngressToken)).
