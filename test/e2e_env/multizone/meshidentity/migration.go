...
func Migration() {
	...
	fromPolicy := fmt.Sprintf(`
type: MeshTrafficPermission
name: test-server-from
mesh: %s
spec:
  targetRef:
    kind: Dataplane
  ...
  - targetRef:
      kind: MeshSubset
      tags:
        kuma.io/service: demo-client_%s_svc
    default:
      action: Allow
`, meshName, namespace)
	...
	BeforeAll(func() {
		Expect(NewClusterSetup().
			...
			Install(YamlUniversal(fromPolicy)).
		...
		NewClusterSetup().
			...
			Install(Parallel(
				testserver.Install(
					...
					testserver.WithServiceAccount("test-server"),
...
`
	...
	issuingIdentity := func(selector string) string {
		return fmt.Sprintf(`
type: MeshIdentity
...
`, meshName, selector)
	...
	// test-server isn't allowed by any policy, its service account differs from demo-client's
	testServerBlocked := func() {
		GinkgoHelper()

		Eventually(func(g Gomega) {
			resp, err := client.CollectFailure(
				multizone.KubeZone1, "test-server", "test-server.meshidentity-migration.svc.kuma-2.mesh.local",
...
`, meshName)
		...
		Eventually(func(g Gomega) {
			isReady, err := isMeshIdentityReady(multizone.KubeZone1, hashedName, true)
			g.Expect(err).ToNot(HaveOccurred())
			g.Expect(isReady).To(BeTrue())

			isReady, err = isMeshIdentityReady(multizone.KubeZone2, hashedName, true)
			g.Expect(err).ToNot(HaveOccurred())
			g.Expect(isReady).To(BeTrue())
		}, "30s", "1s").Should(Succeed())

		// create identity without selecting any Dataplane to create autogenerated certificates
		Expect(NewClusterSetup().
			Install(YamlUniversal(issuingIdentity("{}"))).
			Setup(multizone.Global)).To(Succeed())
		...
		// wait for MeshIdentity to be reconcile
		Eventually(func(g Gomega) {
			isReady, err := isMeshIdentityReady(multizone.KubeZone1, hashedName, false)
			g.Expect(err).ToNot(HaveOccurred())
			g.Expect(isReady).To(BeTrue())

			isReady, err = isMeshIdentityReady(multizone.KubeZone2, hashedName, false)
			g.Expect(err).ToNot(HaveOccurred())
			g.Expect(isReady).To(BeTrue())
		}, "30s", "1s").MustPassRepeatedly(5).Should(Succeed())
...
`
		...
		Expect(NewClusterSetup().
			...
			Setup(multizone.Global)).To(Succeed())
		hashedName = hash.HashedName(meshName, "identity-trust-global")
		Expect(WaitForResource(meshtrust_api.MeshTrustResourceTypeDescriptor, model.ResourceKey{Mesh: meshName, Name: fmt.Sprintf("%s.%s", hashedName, Config.KumaNamespace)}, multizone.KubeZone1, multizone.KubeZone2)).To(Succeed())

		// and
		// 'rules' policy for issued identities next to the 'from' policy for mesh.mTLS certificates
		rulesPolicy := fmt.Sprintf(`
type: MeshTrafficPermission
name: test-server-rules
mesh: %[1]s
spec:
  targetRef:
    kind: Dataplane
  ...
  - default:
      ...
      - spiffeID:
          type: Exact
          value: spiffe://%[1]s.%[2]s.mesh.local/ns/%[4]s/sa/default
      - spiffeID:
          type: Exact
          value: spiffe://%[1]s.%[3]s.mesh.local/ns/%[4]s/sa/default
`, meshName, multizone.KubeZone1.ZoneName(), multizone.KubeZone2.ZoneName(), namespace)
		Expect(NewClusterSetup().
			Install(YamlUniversal(rulesPolicy)).
			Setup(multizone.Global)).To(Succeed())
		hashedName = hash.HashedName(meshName, "test-server-rules")
		Expect(WaitForResource(meshtrafficpermission_api.MeshTrafficPermissionResourceTypeDescriptor, model.ResourceKey{Mesh: meshName, Name: fmt.Sprintf("%s.%s", hashedName, Config.KumaNamespace)}, multizone.KubeZone1, multizone.KubeZone2)).To(Succeed())

		// then 'from' keeps allowing mesh.mTLS certificates
		...
		identitySelected := func() {
			Eventually(func(g Gomega) {
				...
				g.Expect(err).ToNot(HaveOccurred())
				g.Expect(isReady).To(BeTrue())
			}, "30s", "1s").MustPassRepeatedly(10).Should(Succeed())
		}

		// when
		// select only clients, so issued identities talk to servers with mesh.mTLS certificates
		Expect(NewClusterSetup().
			Install(YamlUniversal(issuingIdentity(`
    dataplane:
      matchLabels:
		...
		// then 'rules' allows issued identities
		Consistently(func(g Gomega) {
			g.Expect(recorder.FirstError()).ToNot(HaveOccurred())
		}, "10s", "1s").Should(Succeed())
		testServerBlocked()
