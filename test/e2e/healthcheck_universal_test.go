...
var _ = Describe("Test application HealthCheck on Universal", func() {
	...
	BeforeEach(func() {
		...
		err = EchoServerUniversal(AppModeEchoServer, "default", "universal", echoServerToken, ProxyOnly(), ServiceProbe())(cluster)
		Expect(err).ToNot(HaveOccurred())
		err = DemoClientUniversal(AppModeDemoClient, "default", demoClientToken, ServiceProbe())(cluster)
		Expect(err).ToNot(HaveOccurred())
	})
