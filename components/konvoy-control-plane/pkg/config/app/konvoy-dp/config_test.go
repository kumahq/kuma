...
var _ = Describe("Config", func() {
	...
	Context("with modified environment variables", func() {
		...
		It("should be loadable from environment variables", func() {
			...
			Expect(cfg.Dataplane.Service).To(Equal("demo"))
			Expect(cfg.Dataplane.AdminPort).To(Equal(uint32(2345)))
	...
	It("should have validators", func() {
		...
		// then
		Expect(err).To(MatchError(`Invalid configuration: .ControlPlane is not valid: .XdsServer is not valid: .Address must be non-empty; .Port must be in the range [0, 65535]; .Dataplane is not valid: .Id must be non-empty; .Service must be non-empty; .AdminPort must be in the range [0, 65535]; .DataplaneRuntime is not valid: .BinaryPath must be non-empty; .ConfigDir must be non-empty`))
	})
})
