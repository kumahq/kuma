...
`, port, certDir)
	...
	It("should be possible to run `konvoy-injector run`", func(done Done) {
		// given
		cmd := newRootCmd()
		cmd.SetArgs([]string{"run", "--config-file", configFile.Name()})

		// when
		By("starting the Konvoy Injector")
		...
		// then
		By("waiting for Konvoy Injector to become ready")
		...
		// when
		By("signalling Konvoy Injector to stop")
		close(stopCh)
