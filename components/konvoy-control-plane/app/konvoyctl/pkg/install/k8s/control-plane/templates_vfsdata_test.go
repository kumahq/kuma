...
var _ = Describe("Templates", func() {
	...
	konvoyctlSrcDir := filepath.Join("..", "..", "..", "..")

	controlplaneTemplatesDir := controlplane.TemplatesDir(konvoyctlSrcDir)
	...
	DescribeTable("generated Go code must be in sync with the original template files",
		func(given testCase) {
			...
			Expect(err).ToNot(HaveOccurred())
			// and
			Expect(string(actualContents)).To(Equal(string(given.expectedContents)), "generated Go code is no longer in sync with the original template files. To re-generate it, run `make generate/konvoyctl/install/control-plane`")
		},
