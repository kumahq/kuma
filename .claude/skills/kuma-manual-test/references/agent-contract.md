...
3. **NEVER run bare kubectl, kumactl, or curl commands against the cluster.** Every cluster-interacting command MUST be executed through `"${CLAUDE_SKILL_DIR}/scripts/record-command.sh"`. This is not optional post-hoc logging - the script IS the execution method. Do not run `kubectl get`, then record it. Run it THROUGH record-command.sh. Bare commands will be blocked by hooks and rejected. The only exceptions are commands already wrapped by other tracked scripts (apply-tracked-manifest.sh, capture-state.sh, preflight.sh, cluster-lifecycle.sh). Any `kubectl`, `kumactl`, or `curl` command without `record-command.sh` in the same line is wrong.
...
9. When a suite group provides inline manifest YAML, use it verbatim. Do not change names, namespaces, labels, or any fields. If a manifest fails validation or apply, follow the manifest error handling flow in `references/validation.md`: ask the user via AskUserQuestion whether to (a) fix for this run only, (b) fix in the suite with an amendment log entry, or (c) skip the step. Never silently fix a manifest - the error might indicate a real product bug.
...
13. **The test groups table is the execution plan.** Every group listed in the suite's test groups table MUST be executed. Do not skip groups because they need a different cluster profile than what's currently running. If a group requires multi-zone, tear down single-zone and bring up multi-zone. If that's impractical, ask the user via AskUserQuestion. Only the `skipped groups` field in suite metadata lists groups that should NOT be executed.
