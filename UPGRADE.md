...
When using `MeshService`, we were automatically generating `MeshService` resources in Universal mode. However, due to stricter resource validation, we have decided not to generate the `MeshService` object when a dataplane has a `kuma.io/service` that is not [RFC 1035](https://www.rfc-editor.org/rfc/rfc1035.html) compliant.

If you want `MeshService` objects to be autogenerated by the control plane on Universal, update the `kuma.io/service` tags to valid names. Otherwise, you must create them manually.
