...
Successful `DELETE` or `PUT` requests without any warnings will now get "content-type: application/json" in response header and empty json as response content.
...
### MeshService on Universal are no longer generated for services with invalid `kuma.io/service` values

When using `MeshService`, we were automatically generating `MeshService` resources in Universal mode. However, due to stricter resource validation, we have decided not to generate `MeshService` objects for dataplanes with `kuma.io/service` that are not [RFC 1035](https://www.rfc-editor.org/rfc/rfc1035.html) compliant.

If you want `MeshService` to be autogenerated by the control plane on Universal, update the `kuma.io/service` tag to valid names. Otherwise, you must create the `MeshService` objects manually.
...
The field `healthyPanicThreshold` is deprecated on `MeshHealthCheck` and will be removed in a future release. It is now added to `MeshCircuitBreaker` policy. 
