...
* Applications that are binding to `localhost` won't be reachable anymore. We are changing the default inbound cluster that was always pointing to `localhost` to `ORIGNAL_DST` and we are going to route traffic to the destination for connection. Before upgrade check if your applications are listening on `localhost` and should be exposed. In this case change address on which application listen to `0.0.0.0`. Other option is to define `dataplane.networking.inbound[].serviceAddress` to the address which service is using. Other way is to disable the new behavior by setting `kuma-cp` configuration `KUMA_DEFAULTS_ENABLE_INBOUND_PASSTHROUGH` to false or `defaults.enableInboundPassthrough` to false. The last option is going to be removed in further versions. 
