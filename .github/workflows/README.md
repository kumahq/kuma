...
`distributions` asks that question the other way round: do this run's results still describe this pull request? It compares what the run skipped against the draft state `meta` read live, and fails when a run skipped its gates and the pull request is no longer a draft. That catches a gate that quietly stopped working, and it catches the one way a replayed event can lie - re-run an old draft run after marking the pull request ready and GitHub writes that run's skips over the real results.

Two rules follow from that, and a change here has to keep both. `distributions` is the fan-in: it lists every job in its `needs`, because a job it does not need is a job whose failure it cannot see, and nothing else catches it - it runs on `always()` rather than on its needs having succeeded, so a cancelled run reaches the halt step and names the dependency that died. And each gate this section describes compares a decision as a string against the value that turns work off, so a decision that never arrived falls on the safe side: the tests run and publishing does not. The older `fromJSON(inputs.FULL_MATRIX)` gates in `_build_publish.yaml` spell it differently and are safe differently - an empty value is an evaluation error there, which fails the job rather than passing it.
