...
The gate is always a job-level condition, never a narrowed trigger. A job skipped by a condition reports Success and satisfies a required status check, while a workflow that never fires leaves that check waiting for a report and blocks the pull request for good. That is why `meta` holds the required check. `distributions` is the only required context covering CI and it publishes no check run until all eight of its dependencies finish, so it says nothing for roughly forty minutes after Ready for review starts a run. The green one a draft earned by skipping would stay the newest `distributions` on that commit for all of that time, leaving the pull request mergeable with nothing tested and `auto-merge.yaml` arming on the same event. So `meta` posts a `distributions` check run of its own, `in_progress`, about a minute in. A newer check run of the same name replaces the older one outright - measured, the older one stops being listed at all - so the required check goes pending from then until this run's own `distributions` completes. A pull request from a fork gets a read-only token, so the hold fails there and the step says so rather than failing the job. One exception is worth knowing: a *matrix* job skipped this way reports under its unexpanded name - `test / e2e (default, ${{ matrix.k8sVersion }}, ${{ matrix.arch }})` - so the per-leg e2e names do not report whenever the matrix comes out empty, which `ci/skip-test` has always done and a draft now does too. They come back when the matrix has entries.
