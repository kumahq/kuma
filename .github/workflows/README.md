...
`build-test-distribute` decides in its `meta` job, and every job that reads a label reads that decision. Draft state is the exception: `build_check`, `check` and `meta`'s own gate outputs read it straight from the event, because it is correct there and waiting on `meta` would put an API call in front of a forty minute job. `meta` reads it live as well, but only for the fan-in check below - never to decide what runs. Two things feed it.

## Draft state

A draft runs nothing expensive: `build_check`, `check`, `test_unit`, the whole e2e matrix, and `build_publish` with the container-structure test inside it are all skipped. `distributions` then fails on purpose, so a draft carries one red check saying it has tested nothing. Press **Ready for review** to run them - `ready_for_review` starts a fresh run - and converting back to a draft cancels the run in flight and replaces it with one that skips. `validate-workflows-and-scripts` follows the same rule. `check.yaml` - the "PR health" workflow, not the `check` job above - deliberately does not: it runs commitlint over the pull request title, which becomes the commit message on a squash merge, and it takes seconds on the smallest runner. A wrong title is worth catching on the first push.
...
The gate is always a job-level condition, never a narrowed trigger. A job skipped by a condition reports Success and satisfies a required status check, while a workflow that never fires leaves that check waiting for a report and blocks the pull request for good. That is why `distributions` fails on a draft rather than passing. It is the only required context covering CI, and it publishes no check run until all eight of its dependencies finish, so it does not report again until roughly forty minutes into the run that Ready for review starts. A green one from the draft run would stay the newest `distributions` on that commit for all of that time, and the pull request would be mergeable, with `auto-merge.yaml` arming on the same event. A red one costs nothing - GitHub refuses to merge a draft anyway - and it stays red until the run that earns green finishes. One exception is worth knowing: a *matrix* job skipped this way reports under its unexpanded name - `test / e2e (default, ${{ matrix.k8sVersion }}, ${{ matrix.arch }})` - so the per-leg e2e names do not report whenever the matrix comes out empty, which `ci/skip-test` has always done and a draft now does too. They come back when the matrix has entries.

`distributions` is the fan-in, and a change here has to keep three things. It lists every job in its `needs`, because a job it does not need is a job whose failure it cannot see and nothing else catches it, and it runs on `always()` rather than on its needs having succeeded, so a cancelled run still reaches the halt step and is told which dependency died. It asks whether this run's results still describe this pull request, comparing what the run skipped against the draft state `meta` read live - which catches a gate that quietly stopped working, and catches the one way a replayed event can lie, because re-running an old draft run after marking the pull request ready writes that run's skips over the real results. And each gate this section describes compares a decision as a string against the value that turns work off, so a decision that never arrived falls on the safe side: the tests run and publishing does not. The older `fromJSON(inputs.FULL_MATRIX)` gates in `_build_publish.yaml` are safe differently, by making an empty value an evaluation error that fails the job.

## Labels

`meta` asks the API which labels the pull request carries at the moment it runs, rather than trusting the set the webhook carried - a pull request cannot be created with labels, so they always arrive in a second call and the webhook's copy is routinely empty. A label added before `meta` runs counts; one added after it takes effect on the next run, or on a re-run of this one, since a re-run repeats the request rather than replaying the snapshot. `meta` reaches that step twenty to forty seconds into a run, which is worlds more than a tool needs - `gh pr create --label`, Renovate, `release.yaml` and `backport.yaml` all attach within a second of opening - and not long enough for someone clicking a label by hand. For them the label lands on the next push or a re-run.
