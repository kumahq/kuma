...
`distributions` is the only required check covering CI, and it publishes nothing until all eight of its `needs` finish. A draft's green would therefore stay newest for the forty minutes the ready run takes, long enough to merge with nothing tested. So `meta` posts a `distributions` check of its own, `in_progress`, a minute in: a newer check run of the same name replaces the older one outright, which holds the required check pending until this run reports. A pull request from a fork gets a read-only token, so the hold fails there and the step says so.
