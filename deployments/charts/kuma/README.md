...
| meshes | list | [] | List of meshes to deploy zone proxies for. Each entry renders per-mesh Deployment(s), Service(s), HPA, PDB, and ServiceAccount. Optionally renders a Mesh resource and default policies (unfederated zones only). Default: [] (no mesh-scoped zone proxies deployed). |
