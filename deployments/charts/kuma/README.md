...
| meshes | list | `[]` | List of meshes to deploy zone proxies for. Each entry renders per-mesh Deployment(s), Service(s), HPA, PDB, and ServiceAccount. Example single-mesh configuration: meshes:     # -- The mesh with this name must already exist or be created separately; this Helm chart will not create it   - name: default      ingress:       # -- Deploy a zone ingress for this mesh.       enabled: true       # -- Number of replicas. Ignored when hpa.enabled is true.       replicas: 1       # -- Pause container image (dummy main container for sidecar injection).       # Override if registry.k8s.io is not reachable or mirrored elsewhere.       image: registry.k8s.io/pause:3.10       # -- Override the auto-generated Service name (max 63 chars).       # Auto-generated: <name>-<mesh>-ingress (where <name> is the chart name or nameOverride)       service:         name: ""       # -- Resource requests and limits for the pod (pod-level resources).       # Applied to all containers in the pod (pause + injected kuma-sidecar).       resources: {}       # -- Subset of Kubernetes PodSpec fields applied to the pod template       # (nodeSelector, tolerations, affinity, topologySpreadConstraints,       #  priorityClassName, securityContext, containerSecurityContext).       podSpec: {}       # -- Horizontal Pod Autoscaler settings.       hpa:         enabled: false         minReplicas: 2         maxReplicas: 5         targetCPUUtilizationPercentage: 80       # -- Pod Disruption Budget settings.       pdb:         enabled: false         maxUnavailable: 1      egress:       # -- Deploy a zone egress for this mesh.       enabled: true       replicas: 1       # -- Pause container image (dummy main container for sidecar injection).       # Override if registry.k8s.io is not reachable or mirrored elsewhere.       image: registry.k8s.io/pause:3.10       # -- Override the auto-generated Service name (max 63 chars).       # Auto-generated: <name>-<mesh>-egress (where <name> is the chart name or nameOverride)       service:         name: ""       # -- Resource requests and limits for the pod (pod-level resources).       # Applied to all containers in the pod (pause + injected kuma-sidecar).       resources: {}       # -- Subset of Kubernetes PodSpec fields applied to the pod template       # (nodeSelector, tolerations, affinity, topologySpreadConstraints,       #  priorityClassName, securityContext, containerSecurityContext).       podSpec: {}       hpa:         enabled: false         minReplicas: 2         maxReplicas: 5         targetCPUUtilizationPercentage: 80       pdb:         enabled: false         maxUnavailable: 1 |
