...
| dataPlane.features.otelEnv | bool | `false` | Enables OTEL exporter env-var support in `kuma-dp`. When set to true, it sets the environment variable KUMA_RUNTIME_KUBERNETES_INJECTOR_OTEL_ENV_ENABLED=true in the control plane, which causes the sidecar injector to add KUMA_DATAPLANE_RUNTIME_OTEL_ENV_ENABLED=true to each injected `kuma-sidecar` container. |
