...
Being global-scoped meant they lived outside any mesh context, which created two blockers:
...
1. No mesh identity — `ZoneEgress` could not participate in mTLS with a
   ...
   to derive an identity from. Source-identity-based access control (MeshTrafficPermission)
   could not be enforced at zone proxy boundaries.
...
2. No policy support — the policy system operates on `Dataplane` resources within a mesh.
   ...
   (MeshAccessLog, MeshMetric, MeshTrace, MeshTrafficPermission, MeshRateLimit, MeshFaultInjection)
   could target them.

This MADR establishes the unified policy model for zone proxies:
1. How inbound policies (MeshTrafficPermission, MeshRateLimit, MeshFaultInjection, etc.)
   apply to zone proxy dataplanes
2. How to match specific destinations (MeshExternalService) in inbound rules on zone egress
3. How HTTP-level outbound policies (MeshHTTPRoute, MeshTimeout, MeshRateLimit) work on zone egress
...
* Bad, because `targetRef` in `Match` is only meaningful on zone proxy Dataplanes where
  per-destination filter chains exist. On regular sidecars the field is invalid and must be
  rejected at admission, meaning the shared `Match` struct carries a field that most consumers
  can never use.
